...
MULTIFACTOR = {
    ...
    "FIDO_SERVER_NAME": urlparse(
        SITE_URL
    ).hostname,  # Human-readable name for FIDO request
    "TOKEN_ISSUER_NAME": urlparse(
        SITE_URL
    ).hostname,  # TOTP token issuing name (to be shown in authenticator)
    "FACTORS": ["FIDO2"],
    "FALLBACKS": {},
}
